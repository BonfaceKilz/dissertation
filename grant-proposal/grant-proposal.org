...
Genenetwork (GN) is a long-running bio-medical webservice that has been in existence since 1994[cite:@mulligan2017genenetwork;@sloan2016genenetwork;@wang2003webqtl;https://pubmed.ncbi.nlm.nih.gov/33472826/]. GN enables researchers with little programming or mathematical statistics experience to access omics data through the web interface, and to also run complex statistical models on the aforementioned data vis-a-vis a web interface. For the more advanced user, this data is presented through API end-points (which is accessible in a programming environment such as R or Python), or alternatively inside live Jupyter notebooks residing in self-hosted GNU Guix containers [fn:guix-containers]
