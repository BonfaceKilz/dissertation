...
Effectively, even public data that is shared among different communities is often siloed because relationships between data elements have to be guessed from limited information, such as column headers and row names. This is a time-sink in terms of developer hours in trying to work out how to integrate and use such datasets.

@@ Note: in the above paragraph I would only focus on the machines - so this can be an AI preparation proposal. See:
...
Genenetwork (GN) is a long-running bio-medical webservice that has been in existence since 1994[cite:@mulligan2017genenetwork;@sloan2016genenetwork;@wang2003webqtl;https://pubmed.ncbi.nlm.nih.gov/33472826/]. GN aims to give researchers open-access to FAIR data that they can run analysis on vis-a-vis (@@ not clear): the web; API end-points; and live Jupyter notebooks inside self-hosted GNU Guix containers[cite:@theGuixEnvironment2022Gnu]. GN is uniquely centered on hosting genetics data, phenotyping, QTL and GWA studies in human and model species, such as mouse and rat. Having all this data from different experiments spanning over 20 years in one database with a simple web UI allows any researcher to run analysis and correlate over many studies.
...
REST APIs are not very useful for automated AI analysis because machines can not reason about the relationships of the provided resources.
It is clear that more flexible approaches are needed to increase the value of the service.  Unlocking the data means re-organizing the data and providing a way of automated data discovery.
In addition I aim to make a copy of the current widely used service that is physically hosted in Memphis TN, USA, and host it in Kenya at KEMRI/Wellcome Trust Kilifi.
This will allow us to add African studies and data to GN using the latest methods, including homomorphic encryption of human data [cite:@mott2020private] and differential privacy [cite:@dwork2008differential;https://nlnet.nl/project/VariationGraph/].

Underpinning the GN service is a mountain of data that is growing steadily (currently about 0.5TB), and with it new challenges, in particular: espousing clear relations between the data given a history of ad-hoc storage; fast data retrieval; non-obvious complex queries to do seemingly straight-forward tasks; connecting with other open databases (e.g. Uniprot) in a coherent way; and unclear means of adding new data that does not align with existing schemata. In the GN software model I found XXX specialised SQL queries for search, data fetching and analysis.
Prividing a new graph-based RDF data model with built-in relationships will make machine-based data mining possible[cite]. These methods are not new. Two successful very large and high performing RDF databases are Wikidata - the backend for Wikipedia - and Uniprot [cites].

With this grant I aim to present new opportunities to the scientific community; and in particular, African organizations that have faced similar problems. Having more semantic data that's easily accessible and shareable helps in pushing research work forwards. It would be a good first step in ultimately answering some difficult open biological questions such as: "Are specific genes/genotypes associated with certain cancers in certain populations in Kenya" or "What traits/phenotypes are risk factor in conjunction with diabetes or hypertension in the Kenya"? Suffice to say that these questions can not be answered by some 'foreign' database or association study. Key is to come up with a system that can allow for data analysis. And because GeneNetwork is a long running and successful self-sustained open source project [cite github stats] I am convinced that it is a great fundament for future data science work and that my contribution will be absorbed in the overall project.
