...
Underpinning the GN service is a mountain of data that is growing steadily (currently about 30 TB), and with it new challenges, in particular: espousing clear relations between the data given a history of ad-hoc storage; fast data retrieval; non-obvious complex queries to do seemingly straight-forward tasks; connecting with other open databases (e.g. Uniprot) in a coherent way; and unclear means of adding new data that does not align with existing schemata. In the GN software model, I found 163 specialised SQL queries for search, data fetching and analysis. Providing a new graph-based RDF data model with built-in relationships and machine-readable annotations will make machine-based data mining possible \autocite{munoz2014using}. These methods are not new. Two successful very large and high performing RDF databases are: Wikidata&#x2014;the back-end for Wikipedia&#x2014;and Uniprot \autocite{erxleben2014introducing,malyshev2018getting,redaschi2009uniprot}.

With this grant, I aim to present new opportunities to the scientific community; and in particular, African organisations that have faced similar problems. At a personal level, being prediabetic, it would be encouraging if scientists had easier access to well-annotated localised Kenyan data so that they could perform useful analysis that would enable them to answer questions such as: "Are specific genes/genotypes associated with certain cancers in certain populations in Kenya?" or "What traits/phenotypes are risk factors in conjunction with diabetes or hypertension in the Kenya?" An example of a service that does this as part of the GN infrastructure is GeneCup, a tool that efficiently and comprehensively answers the question: "What do we know about these genes and the topic I study?" \autocite{2022geneCupWeb,gunturkun2022genecup}. As an example of using GeneCup, Figure [10](#orgea90bc6) shows a graph that uses the keyword: *diabetes*, in finding a relation between it and gene symbols from the NHGRI-EBI GWAS catalog. However impressing GeneCup may be, it does not retrieve relationships between genes, but instead focuses on the relationship between genes and a set of keywords organized as an ontology \autocite{gunturkun2022genecup}. Using linked data, particularly in GN, would overcome such limitations, in addition to allowing easier integration with other services that also use RDF.

![img](./genecup-graph.png "Finding relationships between the keyword: *diabetes* and gene symbols. GeneCup does it's literature search by querying the NIH PubMed server programmatically, and fetching the abstracts from a local copy of the PubMed archive, and later uses deep learning to automatically distinguish sentences describing systemic stress from those describing cellular stress \autocite{2022geneCupWeb}")

Suffice to say that the aforementioned questions can not be answered by some 'foreign' database or association study. The key is to come up with a system that can allow for data analysis. And because GN is a long running and successful self-sustained open source project \autocite{gnGitStats} that already tracks omics data related to human diseases and conditions such as diabetes and addiction, I am convinced that it is a great fundament for future data science work and that my contribution will be absorbed in the overall project.

**Research Hypothesis**: Moving graph-like biological data from a SQL database into a graph-like structure, RDF, will make it [the data] more semantic, easier to share, reduce developer hours required during system integration with other data systems, and make it more machine-discoverable.
